...
type WPTResult struct {
	...
	PageSpeedVersion           float32          `json:"pageSpeedVersion"`
	...
	Adult_site                 string           `json:"adult_site"`
	Fixed_viewport             string           `json:"fixed_viewport"`
	...
	userTime                   map[string]int32 `json:"userTime"`
}
...
type WPTResultSet struct {
	Results     WPTResult   `json:"results"`
	Pages       Pages       `json:"pages"`
	Thumbnails  Thumbnails  `json:"thumbnails"`
	Images      Images      `json:"images"`
	RawData     RawData     `json:"rawData"`
	VideoFrames VideoFrames `json:"videoFrames"`
}
...
func GetResult(url string, key string) (result Result) {
	...
	res, err := http.Get(url + "jsonResult.php?id=" + key)
